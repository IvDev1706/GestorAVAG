...
class searchClient(forms.Form):
    ...
    # Clase de formulario para buscar alumno por CURP
class searchClientByCurp(forms.Form):
    curp = forms.CharField(label="CURP del alumno:",
                          max_length=18,
                          widget=forms.TextInput(attrs={"style":"float: right;",
                                                       "placeholder":"CURP (18 digs)"}))

# Clase de formulario para actualizar alumno
class updateClientForm(forms.Form):
    ...
    # Clase de formulario para registrar pago
class registerPaymentForm(forms.Form):
    monto = forms.DecimalField(label="Monto:",
                              max_digits=10,
                              decimal_places=2,
                              widget=forms.NumberInput(attrs={"style":"float: right;",
                                                            "readonly":"readonly"}))
    fecha_pago = forms.DateField(label="Fecha de pago:",
                                widget=forms.DateInput(attrs={"style":"float: right;",
                                                            "type": "date"}))
    tipo = forms.ChoiceField(label="Forma de pago:",
                            choices=[('Efectivo', 'Efectivo')],
                            widget=forms.Select(attrs={"style":"float: right;"}))
